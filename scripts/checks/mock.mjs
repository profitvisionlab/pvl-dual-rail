...
const PROVIDER_ENV = [
  ...
  'TOGETHER_API_KEY', 'DEEPINFRA_API_KEY', 'LIGHTNING_API_KEY', 'OPENROUTER_API_KEY',
  'TOGETHER_BASE_URL', 'DEEPINFRA_BASE_URL', 'LIGHTNING_BASE_URL',
  ...['TOGETHER', 'DEEPINFRA', 'LIGHTNING'].flatMap((p) => [1, 2, 3].map((t) => `${p}_TIER${t}_MODELS`)),
]
...
export async function withEnv(vars, fn) {
  const keys = new Set([...PROVIDER_ENV, ...Object.keys(vars),
    'TOGETHER_BACKOFF_BASE_MS', 'DEEPINFRA_BACKOFF_BASE_MS', 'LIGHTNING_BACKOFF_BASE_MS',
    'TOGETHER_MAX_ATTEMPTS', 'DEEPINFRA_MAX_ATTEMPTS', 'LIGHTNING_MAX_ATTEMPTS'])
  const saved = {}
  for (const k of keys) { saved[k] = process.env[k]; delete process.env[k] }
  const all = {
    TOGETHER_BACKOFF_BASE_MS: '0', DEEPINFRA_BACKOFF_BASE_MS: '0', LIGHTNING_BACKOFF_BASE_MS: '0',
    TOGETHER_MAX_ATTEMPTS: '2', DEEPINFRA_MAX_ATTEMPTS: '2', LIGHTNING_MAX_ATTEMPTS: '2',
    ...vars,
  }
